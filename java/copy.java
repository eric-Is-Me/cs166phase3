    ...
    // Given a hotelID, Staff SSN, roomNo, repairID , date create a repair request in the DB
	   String repairDate;
	      ...
	      try
	      {
	        ...
	        String query_repID = "SELECT MAX(rID) FROM Repair";
	        Statement repID_Statement = esql._connection.createStatement();
	        ResultSet repID_ResultSet = repID_Statement.executeQuery(query_repID);
	        repID_ResultSet.next();
	        int repID_Increment = repID_ResultSet.getInt(1) + 1;
	        String query_repID = "INSERT INTO Repair VALUES ";
	        Statement repID_Statement = esql._connection.createStatement();
	        ResultSet repID_ResultSet = repID_Statement.executeQuery(query_repID);
	        repID_ResultSet.next();
	        query = "INSERT INTO Request VALUES (" + reqID_Increment + ", " + managerID + ", " + repID_Increment + ", '" + repairDate + "'::date)";
	        esql.executeQuery(query);
	      }
	      catch (Exception e)
	      {
   ...
   public static void topKHighestRoomPriceForADateRange(DBProject esql){
    // List Top K Rooms with the highest price for a given date range
      ...
      while (true)
      {
       ...
       try
      {
        query = "SELECT price FROM Booking B, Room R WHERE R.roomNo = B.roomNo AND B.bookingDate BETWEEN '" + date1 + "'::date AND '" + date2 + "'::date ORDER By B.price DESC Limit " + k;
        esql.executeQuery(query);
      }
      catch (Exception e)
      {
